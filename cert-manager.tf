...
resource "null_resource" "cert-manager-crds" {
  depends_on = [kubernetes_namespace.cert-manager]
   ...
   command = "apk add --update --no-cache bash python3 libffi openssl curl && ln -s /usr/bin/python3 /usr/bin/python && curl -L https://aka.ms/InstallAzureCli | bash && curl -s -LO https://storage.googleapis.com/kubernetes-release/release/$(curl -s https://storage.googleapis.com/kubernetes-release/release/stable.txt)/bin/linux/amd64/kubectl && chmod +x ./kubectl && mv ./kubectl /usr/local/bin/kubectl"
